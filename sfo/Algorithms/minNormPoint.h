...
bool setOptCheckDuality(vector<double> x, submodularFunctions& f){
    ...
    double sum = 0;
    A.clear();
    unordered_set<int>::iterator iter = f.getGroundSet().begin();
    for(int i=0;i<x.size();i++, iter++){
      if(x[i] <=0){
        	sum += x[i];
        	A.insert(*iter);
      }
...
bool minNormPoint(submodularFunctions& f, unordered_set<int>& minSolSet, unordered_set<int>& maxSolSet, std::vector<double>& xstar, int verbosity = 1){
	...
	while(1){
		...
		if (earlyStopEnabled > 0 && setOptCheckDuality(xstar, f)){
