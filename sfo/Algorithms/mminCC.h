 ...
 */
#ifndef __MMIN_INCLUDED__
#define __MMIN_INCLUDED__

unordered_set<int> modularmin(vector<double> mupper, double k){
...
std::unordered_set<int> mminCC(submodularFunctions& f, double k, int verbosity = 1, std::unordered_set<int> startSet = unordered_set<int>()){
	...
	while(1){
		...
		newSeta = modularmin(muppera, k);
		newSetb = modularmin(mupperb, k);		
