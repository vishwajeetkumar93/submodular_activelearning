...
class patternCoverage: public submodularFunctions{
	...
	vector<Pattern> finalsetCollection;
...
//	patternCoverage(std::unordered_set<int>& groundSet,int wordCount, char* graphFilet);
	patternCoverage(std::unordered_set<int>& groundSet,vector<Pattern> pcollection,vector<vector<bool>> bitCorpus,vector<Pattern> fCollection);
...
{
		for(vector<Pattern>::iterator it = finalsetCollection.begin(); it != finalsetCollection.end(); it++ ){

		std::vector<Triplet> v_item = (*it).occurances;
...
patternCoverage::	patternCoverage(std::unordered_set<int>& groundSet,vector<Pattern> pcollection,vector<vector<bool>> bitCorpus,vector<Pattern> fCollection) : submodularFunctions(groundSet), patternCollection(pcollection),bitCorpus_preCompute(bitCorpus),bitCorpus_empty(bitCorpus),finalsetCollection(fCollection){
...
/*
	...
	return (double (gains));
	* 	* **/
   cout<< "Inside patternCoverage::evalGainsremove\n";
   return 0;
}
...
// If safe = 0, (for the sake of speed) we do not check if item belong to the subset. We also do not check if the preComputed statistics actually points to the set sset. You would need to check both these.
	...
	return (double(count));
*/
cout <<"Inside patternCoverage::evalGainsremoveFast \n";
return 0;
}
...
*/
cout <<"Inside patternCoverage::updateStatisticsRemove n";
}
