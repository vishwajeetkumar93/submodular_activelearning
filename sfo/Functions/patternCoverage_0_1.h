 ...
 */
class patternSentenceCoverage: public submodularFunctions{
	protected:
	...
	vector<int> sentenceData; // stores the wordCount of sentence. i-th integer stores the wordcount of i-th sentence
	...
	vector<bool> preComputeSentence // stores whether the ith sentence is covered or not
	...
	int sizePreComputeSentence; // no. of sentences in the corpus for
	unordered_set<int> preComputeSet; // This points to the preComputed Set for which the statistics p_X is calculated.
	public:
	// Functions
	patternSentenceCoverage(std::unordered_set<int>& groundSet,int wordCount, char* graphFilet);
	patternSentenceCoverage(std::unordered_set<int>& groundSet,int wordCount, vector<vector <int>> kernel,vector<int> pattern2Length);
	...
	void testDisplay();
	virtual ~patternSentenceCoverage(){}
};

patternSentenceCoverage::patternSentenceCoverage(std::unordered_set<int>& groundSet,int sentenceCount,int w,char *sentenceMapFile char* graphFile): submodularFunctions(groundSet){
	readfromFile(graphFile);
	LoadSentenceMap(sentenceMapFile);
	sizepreCompute = w;
	sizePreComputeSentence=sentenceCount;
	preCompute.assign(sizepreCompute, 0); //precomputed statistics for speeding up greedy
}

patternSentenceCoverage::LoadSentenceMap()

patternSentenceCoverage::patternSentenceCoverage(std::unordered_set<int>& groundSet,int w, vector<vector <int>> kernel,vector<int> pattern2Length) : submodularFunctions(groundSet), patternsData(kernel),pattern2Length(pattern2Length){
	sizepreCompute = w;
	preCompute.assign(sizepreCompute, 0); //precomputed statistics for speeding up greedy
}
void patternSentenceCoverage::readfromFile(char* graphFile){
...
void patternSentenceCoverage::testDisplay(){
	/*for(int i=0;i<n;i++){
...
double patternSentenceCoverage::eval(const unordered_set<int>& sset){
	int count = 0;
	int pLen=0;
	int i=0;
	int tmp;
...
double patternSentenceCoverage::evalFast(const unordered_set<int>& sset, bool safe=0){
	if(safe == 1){
...
double patternSentenceCoverage::evalGainsadd(const unordered_set<int>& sset, int item){
	double gains = 0;
	int count1 = 0;
	int count2 = 0;
	int pLen=0;
...
double patternSentenceCoverage::evalGainsremove(const unordered_set<int>& sset, int item){
...
double patternSentenceCoverage::evalGainsaddFast(const unordered_set<int>& sset, int item, bool safe=0){
...
double patternSentenceCoverage::evalGainsremoveFast(const unordered_set<int>& sset, int item, bool safe=0){
...
void patternSentenceCoverage::updateStatisticsAdd(const unordered_set<int>& sset, int item, bool safe=0){
...
void patternSentenceCoverage::updateStatisticsRemove(const unordered_set<int>& sset, int item, bool safe=0){
...
void patternSentenceCoverage::setpreCompute(const unordered_set<int>& sset){
	clearpreCompute();
	unordered_set<int>::const_iterator it;
	for( it = sset.begin(); it != sset.end(); it++ ){
...
void patternSentenceCoverage::clearpreCompute(){
